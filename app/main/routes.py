...
from app.models import Subvention, LigneBudget, Depense, Projet, SubventionProjet
...
def dashboard():
    if current_user.role == "admin_tech":
        return render_template("dashboard.html", mode="admin_tech", subs=[], alertes_bilan=[])
    ...
    # Alertes "blindage" : indicateurs de cohérence
    alertes = []
    for s in subs:
        ...
        # 1) Reçu > réel lignes => pas ventilé (ou ventilé partiellement)
        ...
        # 2) Engagé > réel lignes => dépenses qui dépassent l'enveloppe ventilée
        if reel_lignes > 0 and engage > reel_lignes:
            alertes.append(f"{s.nom} : engagé {engage:.2f}€ > lignes réel {reel_lignes:.2f}€ (dépassement).")

        # 3) Info “lecture” : réel lignes > reçu (pas forcément grave, mais utile)
        if reel_lignes > recu and recu > 0:
            alertes.append(f"{s.nom} : lignes réel ({reel_lignes:.2f}€) > reçu ({recu:.2f}€).")

    mode = current_user.role
    return render_template("dashboard.html", mode=mode, subs=subs, alertes_bilan=alertes)
...
def stats():
    ...
    if current_user.role == "responsable_secteur":
        # Forcer le secteur = assignation, ignorer tout filtre manuel
    ...
    # Pré-calcul des années disponibles (pour sélecteur) : toutes les années présentes dans les subventions actives
...
def bilan_global():
    ...
    # --- BLINDAGE RESPONSABLE SECTEUR : on force le secteur ---
    if current_user.role == "responsable_secteur":
        selected_secteur = current_user.secteur_assigne
        selected_projet_id = None  # optionnel : tu peux autoriser le filtre projet, mais seulement sur SON secteur
